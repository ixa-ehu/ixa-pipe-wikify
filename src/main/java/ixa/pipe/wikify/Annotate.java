...
public class Annotate {
	...
	public void wikificationToKAF (KAFDocument kaf, String host, String port, double confidence, int support, boolean coreference) throws Exception {
		String text = "";
		...
		Document response = c.extract(new Text(text), host, port, confidence, support, coreference);
		XMLSpot2KAF(kaf, response);
	}

	public void wikificationToKAF (KAFDocument kaf, String host, String port) throws Exception {
		wikificationToKAF(kaf, host, port, DBpediaSpotlightClient.CONFIDENCE, DBpediaSpotlightClient.SUPPORT, DBpediaSpotlightClient.COREFERENCE);
	}
