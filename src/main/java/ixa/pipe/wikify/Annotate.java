...
public class Annotate {
    ...
    public void wikificationToKAF (KAFDocument kaf, String host, String port, boolean coreference, double confidence, int support) throws Exception {
        String text = "";
        ...
        Document response = annotate(text, host, port, coreference, confidence, support);
        XMLSpot2KAF(kaf, response);

    }


    private Document annotate(String text, String host, String port, boolean coreference, double confidence, int support) throws AnnotationException {
        Document response = c.extract(new Text(text), host, port, coreference, confidence, support);
        return response;
    }


    private void XMLSpot2KAF(KAFDocument kaf, Document spotDoc){
        String resourceExternalRef = "DBpedia";
        String sourceMarkable = "DBpedia_spotlight";
